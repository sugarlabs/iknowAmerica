# -*- coding: utf-8 -*-

from gettext import gettext as _

NAME = _('Cuba')

STATES = [
    (_('Pinar del Río'), 254, 105, 357, 30),
    (_('Havana'), 253, 213, 333, 0),
    (_('Cienfuegos'), 252, 324, 382, 0),
    (_('Matanzas'), 250, 275, 350, 0),
    (_('Sanctí Spiritus'), 249, 394, 404, 45),
    (_('Villa Clara'), 248, 357, 349, -30),
    (_('Camagüey'), 247, 505, 445, 0),
    (_('Ciego de Ávila'), 246, 450, 402, 45),
    (_('Guantánamo'), 245, 716, 529, 0),
    (_('Las Tunas'), 244, 572, 468, 45),
    (_('Granma'), 243, 577, 527, 0),
    (_('Santiago de Cuba'), 242, 616, 558, 0),
    (_('Holguín'), 241, 626, 484, 0),
    (_('Isla de la Juventud'), 240, 156, 447, 0),
    (_('Cayman Islands'), 239, 312, 581, 0),
    (_('United States'), 238, 308, 134, 0),
    (_('Andros Island'), 237, 501, 216, -60),
    (_('New Providence'), 236, 538, 154, -45),
    (_('Eleuthera'), 235, 599, 156, -45),
    (_('Cat Island'), 234, 665, 216, 0),
    (_('San Salvador'), 233, 716, 236, 0),
    (_('Great Exuma'), 232, 613, 273, -45),
    (_('Long Island'), 231, 690, 306, -45),
    (_('Crooked Island'), 230, 737, 329, -45),
    (_('Haiti'), 229, 754, 662, 90),
    (_('Jamaica'), 228, 545, 685, 0),
    (_('Honduras'), 227, 87, 857, 0),
    (_('Grand Bahama Island'), 226, 444, 68, 0),
    (_('Abaco Islands'), 225, 591, 48, 90)
]

CAPITALS = [
    (_('Havana'), 195, 311, 0, -30, -14),
    (_('Camagüey'), 502, 442, 1, 0, 14),
    (_('Ciego de Ávila'), 443, 408, 1, 60, 0),
    (_('Cienfuegos'), 327, 381, 1, -55, 0),
    (_('Guantánamo'), 696, 530, 1, 20, 14),
    (_('Holguín'), 619, 477, 1, 0, -14),
    (_('Las Tunas'), 570, 473, 1, 0, 14),
    (_('Manzanillo'), 561, 519, 1, -60, 0),
    (_('Matanzas'), 249, 316, 1, 15, -14),
    (_('Pinar del Río'), 100, 361, 1, 40, 14),
    (_('Sancti Spíritus'), 395, 392, 1, -40, 14),
    (_('Santa Clara'), 360, 364, 1, 25, -14),
    (_('Santiago de Cuba'), 651, 540, 1, 0, 14)
]

CITIES = [
    (_('Amancio'), 526, 484, 2, -20, -14),
    (_('Banes'), 657, 471, 2, 35, 0),
    (_('Baracoa'), 741, 514, 2, 10, 14),
    (_('Bayamó'), 592, 516, 2, 10, 14),
    (_('Cárdenas'), 274, 317, 2, 40, 0),
    (_('Florida'), 481, 430, 2, -45, 0),
    (_('Güines'), 217, 331, 2, -20, 14),
    (_('Guane'), 73, 380, 2, -10, 14),
    (_('Isabela de Sagua'), 354, 328, 2, 70, 0),
    (_('Jovellanos'), 275, 333, 2, 0, 14),
    (_('La Fe'), 164, 412, 2, 0, 14),
    (_('Mariel'), 156, 320, 2, -40, 0),
    (_('Mayarí'), 659, 493, 2, 0, 14),
    (_('Moa'), 711, 493, 2, 25, 0),
    (_('Morón'), 452, 387, 2, 35, 0),
    (_('Nueva Gerona'), 161, 403, 2, 0, -14),
    (_('Nuevitas'), 546, 428, 2, 40, 0),
    (_('Pilón'), 544, 551, 2, 0, 14),
    (_('Placetas'), 380, 372, 2, 45, 0),
    (_('Puerto Padre'), 595, 455, 2, 40, -14),
    (_('San Cristobal'), 144, 340, 2, -70, 0),
    (_('Santa Cruz del Sur'), 497, 490, 2, -85, 0)
]

ROUTES = []

STATS = [
    (_('Capital:'), _('Havana') + _("(23º80' N - 82º23' W)")),
    (_('Language:'), _('Spanish')),
    (_('Government:'), _('Communist state')),
    (_('President:'), _('Raúl Castro')),
    (_('Vice President:'), _('Miguel Díaz-Canel')),
    (_('Independence:'), _('from United States')),
    ('', _('May 20, 1902')),
    (_('Area:'), '%(sup)s %(u)s (%(p)s)' % {'sup': _('110.860'), 'u': _('km²'), 'p': _('105th')}),
    (_('Population:'), '%(v)s (%(p)s)' % {'v': _('11.242.621'), 'p': _('71th')}),
    (_('GDP:'), '%(c)s %(v)s %(u)s (%(p)s)' % {'c': _('USD'), 'v': _('78.694'), 'u': _('billion'), 'p': _('64th')}),
    (_('HDI:'), '%(l)s - %(v)s (%(p)s)' % {'l': _('High'), 'v': _('0,769'), 'p': _('67th')}),
    (_('Currency:'), _('Cuban peso')),
    (_('Updated:'), _('April 5, 2016'))
]

