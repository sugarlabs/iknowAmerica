# -*- coding: utf-8 -*-

from gettext import gettext as _

NAME = _('Saint Vincent and the Grenadines')

STATES = [
    (_('Saint David'), 254, 176, 173, 60),
    (_('Carlotte'), 253, 293, 271, 80),
    (_('Saint Andrew'), 252, 112, 409, 30),
    (_('Saint Patrick'), 251, 106, 317, 20),
    (_('Saint George'), 250, 197, 484, 0),
    (_('Bequia Island'), 249, 594, 75, 0),
    (_('Quatre Island'), 248, 551, 169, 0),
    (_('Mustique Island'), 247, 670, 300, 0),
    (_('Bettowia Island'), 246, 762, 110, 90),
    (_('Baliceaux Island'), 245, 714, 188, 0),
    (_('Pillories Island'), 244, 680, 240, 0),
    (_('Petit Mustique Island'), 243, 653, 372, 0),
    (_('Savan Island'), 242, 627, 422, 0),
    (_('Petit Canouan Island'), 241, 518, 452, 0),
    (_('Canouan Island'), 240, 437, 564, 0),
    (_('Mayreau Island'), 239, 320, 704, 90),
    (_('Union Island'), 238, 246, 773, 0),
    (_('Prune Island'), 237, 318, 808, 45),
    (_('Petit Saint Vicent Island'), 236, 332, 871, 0)
]

CAPITALS = [
    (_('Kingstown'), 132, 488, 0, 0, -14),
    (_('Barrouallie'), 35, 317, 1, 20, -14),
    (_('Chateaubelair'), 101, 212, 1, 0, -14),
    (_('Georgetown'), 351, 225, 1, 0, -14),
    (_('Layou'), 41, 394, 1, 20, -14),
    (_('Port Elizabeth'), 589, 70, 1, 0, -14)
]

CITIES = [
    (_('Ashton'), 249, 780, 2, 0, -14),
    (_('Biabou'), 306, 404, 2, 0, -14),
    (_('Calliaqua'), 197, 537, 2, 0, -14),
    (_('Derrick'), 563, 112, 2, 0, -14),
    (_('Dovers'), 672, 280, 2, 0, -14),
    (_('Fancy'), 245, 19, 2, 0, 14),
    (_('Mesopotamia'), 250, 440, 2, 0, 14),
    (_('Orange Hill'), 344, 148, 2, 0, -14),
    (_('Stubbs'), 268, 502, 2, 0, -14),
    (_('Wallibou'), 137, 148, 2, 0, -14)
]

RIVERS = []

ROUTES = []

STATS = [
    (_('Capital:'), _('Kingston') + _("(13º10' N - 61º14' W)")),
    (_('Language:'), _('English')),
    (_('Government:'), _('Constitutional monarchy')),
    (_('Monarch:'), _('Queen Elizabeth II')),
    (_('Governor:'), _('Frederick Ballantyne')),
    (_('Independence:'), _('from United Kingdom')),
    ('', _('27 of october of 1979')),
    (_('Area:'), _('389 km² (206th)')),
    (_('Population:'), _('103.000 (196th)')),
    (_('GDP:'), _('USD 0.695 billion (181st)')),
    (_('HDI:'), '%(l)s - %(v)s (%(p)s)' % {'l': _('Medium'), 'v': _('0,720'), 'p': _('97th')}),
    (_('Currency:'), _('Caribbean Dollar')),
    (_('Info updated:'), _('5 of april of 2016'))
]

